...
def run_core(res_dict, raw_dir, crop_dir, mask, ag_div, nr_con_px, msk_thr, prd_file):
  ...
  if not os.path.exists(img_file) or not os.path.exists(prd_file):
    print(f'Error - IMG 3071 not found {patient_id}')
    print(f'{img_file} \n{prd_file}')
    return

  nrrd_file = os.path.join(raw_dir, patient_id + '_img.nrrd')
  if not os.path.exists(nrrd_file):
    print(f'Error - IMG not found {patient_id} {nrrd_file}')
    return
  ...
  if mask:
    ...
    if not os.path.exists(msk_file):
      print(f'Error - MSK not found {patient_id} {msk_file}')
      return
...
if __name__ == "__main__":
  ...
  parser.add_argument('--conf',
                      ...
                      help = 'Specify the YAML configuration file containing the run details.' \
                              + 'Defaults to "step4_cac_scoring.yaml"',
  ...
  print(f'Found {len(pred_files)} patients under "{step3_inferred_dir_path_npy}"')
  ...
  print(f'\nSaving results to csv file under "{cacscore_data_path}"')
  ...
  if res_dict:
    ...
    print('\nNo results to calculate mean dice score.') 